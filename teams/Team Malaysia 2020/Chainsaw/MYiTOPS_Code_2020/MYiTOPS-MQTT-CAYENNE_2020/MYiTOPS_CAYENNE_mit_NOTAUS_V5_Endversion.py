...
import os      #Access for shutting down          
...
class Servo:
    def __init__(self, pi, GPIO, Nullposition = 1500, min_Winkel = -20, max_Winkel = 20):   # Nullpostion gibt die Mittelstellung in Pulsweite an, durch Tests bestimmt
    ...
    def gas_prozent(self, prozent): # Gassteuerung
        ...
        self.Position = self.min_Winkel + (prozent*(self.max_Winkel-self.min_Winkel))/100
...
class Not_Aus_class:    # Emergency stop button
    ...
    def _cbf(self, gpio, level, tick):
        ...
        if level == 0:
            ...
            Kaltstart_Flag = False
            client.virtualWrite(15,0,"digital_sensor" , "d")
            Warmstart_Flag = False
            client.virtualWrite(16,0,"digital_sensor" , "d")
            Betrieb_Flag = False
            client.virtualWrite(17,0,"digital_sensor" , "d")
            Aus_Flag = False
...
def on_message(message):
    ...
    elif message.channel==8 :  #channel for aus
        if message.value=="1": 
            Aus_Flag = True
        elif message.value=="0":
            Aus_Flag = False
    ...
    elif message.channel==9 :    #channel for start/stop engine button
        if message.value=="1": 
            StartStopSignalButton_Flag = True
        elif message.value=="0":
            StartStopSignalButton_Flag = False
            
    elif message.channel==10 :  #channel for on/off test profile button
        if message.value=="1":
            TestProfile = False
    ...
    elif message.channel==11 :  #channel for shut down button
        if message.value=="1": 
            ShutDown_Flag = True
        elif message.value=="0":
            ShutDown_Flag = False
            
    elif message.channel==31 :  #channel for throttle
        Throttle = float(message.value)
            
    elif message.channel==32 :  #channel for throttle 1 slider
        Throttle_1 = float(message.value)
        
    elif message.channel==33 :  #channel for throttle 2 slider
        Throttle_2 = float(message.value)
        
    elif message.channel==34 :  #channel for throttle 3 slider
        Throttle_3 = float(message.value)
        
    elif message.channel==35 :  #channel for time 1 slider
        Time_1 = float(message.value)
        
    elif message.channel==36 :  #channel for time 2 slider
        Time_2 = float(message.value)
        
    elif message.channel==37 :  #channel for time 3 slider
...
def Test_Profile():
    ...
    if (Counter_1<Time_1):
        ...
        client.virtualWrite(25, frequence_Reader.RPM, "analog_actuator", "null") # writing to RPM 1 cayenne
        client.virtualWrite(40, frequence_Reader.RPM, "g", "rpm") #writing to the RPM test profile
        client.virtualWrite(41, Throttle_1, "analog_sensor", "null") #writing to Throttle test profile
        sleep(1)
        Counter_1 += 1 # time increases until it reaches Counter_1
        
    if (Counter_1==Time_1 and Counter_2<Time_2):
        Gas_Servo.gas_prozent(Throttle_2) 
        client.virtualWrite(26, frequence_Reader.RPM, "analog_actuator", "null") # writing to RPM 2 cayenne
        client.virtualWrite(40, frequence_Reader.RPM, "g", "rpm") #writing to the RPM test profile
        client.virtualWrite(41, Throttle_2, "analog_sensor", "null") #writing to Throttle test profile
        sleep(1)
        Counter_2 += 1 # time increases until it reaches Counter_2
        
    if (Counter_1==Time_1 and Counter_2==Time_2 and Counter_3<Time_3):
        Gas_Servo.gas_prozent(Throttle_3)
        client.virtualWrite(27, frequence_Reader.RPM, "analog_actuator", "null") # writing to RPM 3 cayenne
        client.virtualWrite(40, frequence_Reader.RPM, "g", "rpm") #writing to the RPM test profile
        client.virtualWrite(41, Throttle_3, "analog_sensor", "null") #writing to Throttle test profile
        sleep(1)
        Counter_3 += 1 # time increases until it reaches Counter_3

    if (Counter_1==Time_1 and Counter_2==Time_2 and Counter_3==Time_3):
        TestProfile = True #user has to repress test profile again to run the test test profile again
        Gas_Servo.move(Winkel_Gas_min) 
        client.virtualWrite(20,1,"digital_sensor" , "d") # set the timout test profile to cayenne
        Counter_1 = 0 # set all counters back to 0
        Counter_2 = 0
        Counter_3 = 0
...
def connect(host='http://google.com'):
    try:
        urllib.request.urlopen(host) #open the google if there is internet connection
        return True
    except:
        #if no internet it does this 
        Gas_Servo.ausschalten()
        Chokehebel_fahren_Servo.ausschalten()
        Chokehebel_drehen_Servo.ausschalten()
        pi.write(Gas_Relais_GPIO, 0)
...
Gas_Servo = Servo(pi, Gas_GPIO,1500, Winkel_Gas_min, Winkel_Gas_max)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            connect() #Checking for internet connection
            
            #Shutdown when shutdown_Flag is true
            if ShutDown_Flag:
                os.system('sudo shutdown -h now')
            
            # Program restarts after timeout for 400 seconds
            if (Connection_Timer > 0) and (Connection_Timer + 400 < time()):
                Time_out()
            ...
            if OnOff_Flag:
                if MotorSaegeButton_Flag:
                    if not ChangeMode_Flag and KaltstartButton_Flag and not WarmstartButton_Flag and not BetriebButton_Flag and not Kaltstart_Flag and not StartStopSignalButton_Flag and not Not_Aus_Flag and not Motor_an_Flag:
                        ...
                        Kaltstart.start()
                        client.virtualWrite(15,1,"digital_sensor" , "d")
                        ...
                        Warmstart.start()
                        client.virtualWrite(16,1,"digital_sensor" , "d")
                        ...
                        Betrieb.start()
                        client.virtualWrite(17,1,"digital_sensor" , "d")
                        
                    elif not ChangeMode_Flag and not BetriebButton_Flag and not KaltstartButton_Flag and not WarmstartButton_Flag and Aus_Flag and not StartStopSignalButton_Flag and not Not_Aus_Flag and not Motor_an_Flag:
                        ChangeMode_Flag = True
                    ...
                    if (Kaltstart_Flag or Warmstart_Flag or Betrieb_Flag) and not Not_Aus_Flag and TestProfileButton_Flag and Throttle == 0 and not TestProfile:
                        Test_Profile()
