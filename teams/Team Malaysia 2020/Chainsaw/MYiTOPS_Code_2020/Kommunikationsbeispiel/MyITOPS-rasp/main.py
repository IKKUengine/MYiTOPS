...
settings["broker"]=brokers[5] # Stored array in dictionary. brokers[0]= "localhost"
...
# used topics
topicEngineControl = "HsKA/IKKULab/IKKULab043/CPS/EngineControl"
topicEngineSensorInfo = "HsKA/IKKULab/IKKULab043/CPS/EngineSensorInfo"

# subsribed topics
settings["topics"]=[(topicEngineControl, qosExactlyOnce)]
...
def sendSensorInfo():
    ...
    frequence_Reader.RPM =  random.randrange(1, 10000)    
    Connection_Counter += 1

    # Change the value inside here
    publisher_dict = {"Publisher": "R.Pi-MYITOPS", 
                      "Connection_Counter": str(Connection_Counter),
                      "Motor_an_Flag": str(Motor_an_Flag),
                      "Zustand": str(Zustand.value),
                      "RPM": str(frequence_Reader.RPM),
                      "Not_Aus_Flag": str(Not_Aus_Flag) }

    publisher_json = json.dumps(publisher_dict)
#    print(publisher_json)

    client.publish(topicEngineSensorInfo, publisher_json, qos=qosExactlyOnce)
...
def on_message(client, userdata, message):
   ...
   if message.topic == topicEngineControl:
       print("received message:")
       Publisher = subscriber_dict["Publisher"]
       Device = subscriber_dict["Device"]
       ControlMode = int(subscriber_dict["ControlMode"])
       StartStopSignal = int(subscriber_dict["StartStopSignal"])
       Throttleposition = int(subscriber_dict["Throttleposition"])

       print(Publisher)
       print(Device)
       print(ControlMode)
       print(StartStopSignal)
       print(Throttleposition)
#   print (str(message.payload.decode("utf-8", "ignore")))
