    ...
    try:
        while True:
            ...
            if Connection_Timer + 0.2 < time():
                ...
                #update_output_all()                                                                              #UPDATE INFORMATION TO THE DATABASE, MEANING THAT IN YOU CASE, YOU NEED TO PUBLISH TO A TOPIC HERE, TO SEND THE INFORMATION TO ANOTHER DEVICE, IN YOUR CASE, IS DEVICE WHICH RUNS THE GUI
            ...
            if OnOff_Flag:
                if MotorSaegeButton_Flag:
                    if not ChangeMode_Flag and KaltstartButton_Flag and not WarmstartButton_Flag and not BetriebButton_Flag and not Kaltstart_Flag and not StartStopSignalButton_Flag and not Motor_an_Flag:
                        ChangeMode_Flag = True
                        Kaltstart = Kaltstart_Thread()
                        Kaltstart.start()
                        client.virtualWrite(18,1,"digital_sensor" , "d")
                        
                    elif not ChangeMode_Flag and WarmstartButton_Flag and not KaltstartButton_Flag and not BetriebButton_Flag and not Warmstart_Flag and not StartStopSignalButton_Flag  and not Motor_an_Flag:
                        ChangeMode_Flag = True
                    ...
                    elif not ChangeMode_Flag and BetriebButton_Flag and not KaltstartButton_Flag and not WarmstartButton_Flag and not Betrieb_Flag and not StartStopSignalButton_Flag and not Motor_an_Flag:
